    ...
    std::function<void()>   restartHandler)
{
    ...
    this->debugModeToggle      = MenuHelper::getAsciiButton("Toggle", [this] {
        this->config->isDebug = !this->config->isDebug;
    });
    this->easyModeToggle       = MenuHelper::getAsciiButton("Toggle", [this] {
        this->config->isEasyMode = !this->config->isEasyMode;
    });
    ...
    this->addStoreDelay = ftxui::Button(
        "+",
        [this] {
            this->config->storeDelay += this->storeDelayStep;
        },
    ...
    this->addSoftDropDelay = MenuHelper::getAsciiButton("+", [this] {
        this->config->softDropDelay += this->softDropDelayStep;
    });
    ...
    this->addUpdatesPerSecond = MenuHelper::getAsciiButton("+", [this] {
        this->config->updatesPerSecond += this->updatesPerSecondStep;
    });
    ...
    this->addSoftDropGravity = MenuHelper::getAsciiButton("+", [this] {
        this->config->softDropGravity += this->softDropGravityStep;
    });
    ...
    this->renderer = ftxui::Renderer(this->inputs, [this] {
        ...
        std::function<bool(int)> subLevelValidator = [](int value) -> bool {
            return value > 1;
        };
        std::function<bool(int)> addLevelValidator = [](int value) -> bool {
            return value < 15;
        };
        std::function<bool(double)> subUpdatesPerSecondValidator = [](double value) -> bool {
            return value > 1;
        };
        ...
        ftxui::Element element = ftxui::vbox({
            ...
            ftxui::vbox({
                ...
                MenuHelper::getToggleOptionElement(
                    "Background",
                    this->config->withBackgroundColor,
                    this->backgroundModeToggle,
                    width),
